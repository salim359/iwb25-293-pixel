    ...
    public function getallFlashcards(int topicId,http:Request req) returns Flashcard[]|sql:Error|UnauthorizedError|NotFoundError {
        // Step 0: Validate JWT token
        jwt:Payload|UnauthorizedError authResult = Authorization(req);
        if (authResult is UnauthorizedError) {
            return authResult; // UnauthorizedError (includes expiry, invalid, or missing token)
            ...
            if userIDResult is int {
                ...
                if (userID == userId) {
                    ...
                    UnauthorizedError unauthorizedError = {
                        body: {message: "User is not authorized to access this ", details: "User ID does not match", timestamp: time:utcNow()}
                    };
