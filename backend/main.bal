...
service /pixel on pixelListener {
    ...
    resource function post quizzes/evaluate(http:Request req) returns json|UnauthorizedError|NotFoundError|error {
        return evaluatequiz(req);
    }
   ...
   // Generate exam questions for a PDF
    ...
    // 1 by 1
    resource function post pdfs/[int pdfId]/evaluatequestion(http:Request req) returns json|NotFoundError|UnauthorizedError|error? {
        return evaluateExam(pdfId, req);
    }
